...
def parse_args():
    ...
    # custom
    p.add_argument('--kld-scale', default=2.0, type=float,
                   help='Maximum margins of dissimilar samples when training contrastive autoencoder.')
    p.add_argument('--margin-between-b-and-m', default=2.0, type=float,
                   help='Maximum margins of dissimilar samples when training contrastive autoencoder.')
    p.add_argument('--sleep', type = int, default = 0, help='Whether to do active learning.')
    ...
    p.add_argument('--is-enc-kld-custom-mid', default=0, type=int,
                   help='')
    p.add_argument('--centroid-type', nargs='?', default=None, choices=['fam', 'bin'],
                   help='')
    ...
    p.add_argument('--kld_anneal_init', default=0.1, type=float,
                   help='')
    p.add_argument('--kld_anneal_warm', default=0.2, type=float,
                   help='')
    ...
    p.add_argument('--loss_func', default='hi-dist-xent',
            choices=['triplet',
                'triplet-mse',
                'triplet-mse-kld-ensemble-xent',
                'triplet-mse-xent',
                'hi-dist-xent',
                'hi-dist-kld-custom-xent-ensemble2',
                'hi-dist-vae-custom-xent-ensemble3',
                'hi-dist-kld-custom-xent-ensemble4',
                'hi-dist-kld-custom-xent-ensemble5',
                'hi-dist-kld-custom-xent-ensemble6',
                'hi-dist-kld-custom-xent-ensemble7',
                'hi-dist-kld-custom-xent-ensemble8',
                'triplet-kld-ensemble-xent',
                'triplet-xent'],
    ...
    p.add_argument('--ood_kld_ensemble', action='store_true', help='Use CAE OOD score to help sampling')
    ...
    p.add_argument('--log_path', type=str,
                   help='log file name.')
    p.add_argument('--retrain-first', default=1, type=int,
    ...
    p.add_argument('--triplet-lambda', default=1.0, type=float,
                   help='lambda in the loss function of contrastive autoencoder.')
...
def adjust_learning_rate(args, optimizer, epoch, warm = False):
    ...
    # the larger the epoch , the smaller the lr
    # I think it's make we don't rely on last dataset
...
def log_nvidia_smi(gpu_id=0):
    ...
    logging.info(f"nvidia-smi ▶ used={used_mb:4d}MB / total={total_mb:4d}MB")
