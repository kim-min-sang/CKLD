...
class EncKldCustomMlpOnly6(nn.Module):
    def __init__(self, enc_dims, mlp_dims, kld_scale = None, kld_dev_scale = None, dropout=0.2, verbose=1):
        super().__init__()
        self.enc_dims = enc_dims
        self.mlp_dims = mlp_dims
        ...
        # MLP
        self.mlp_dims[0] = self.mlp_dims[0] * 1
        m_stacks = len(self.mlp_dims) - 1
        for i in range(m_stacks - 1):
            self.mlp_modules.append(nn.Linear(self.mlp_dims[i], self.mlp_dims[i + 1]))
            self.mlp_modules.append(nn.ReLU())
    ...
    def eval_plot(self, x_tensor_cuda, y_bin, epoch, args):
        with torch.no_grad():
            ...
            for i in range(0, c_encoded.shape[1], 2):
                ...
                directory_path = "./maps/hc_kld_only/"+ str(c_encoded.shape[1]) + "/epoch_" + str(epoch)
                # 디렉토리 생성
                if not os.path.exists(directory_path):
                    os.makedirs(directory_path)
    ...
    def forward(self, x):
        ...
        #z_concat = torch.cat((c_encoded, kld_encoded), dim=1)
        #mlp_out = self.mlp_model(z_concat)
        mlp_out = self.mlp_model(kld_encoded)
...
class CAEKldOnlyMlp(nn.Module):
    ...
    def eval_plot(self, x_tensor_cuda, y_bin, epoch, args):
        with torch.no_grad():
            ...
            for i in range(0, x_encoded.shape[1], 2):
                ...
                directory_path = "./maps/cade_kld_only/"+ str(x_encoded.shape[1]) + "/epoch_" + str(epoch)
                if not os.path.exists(directory_path):
                    os.makedirs(directory_path)
    ...
    def forward(self, x):
        ...
        #z_concat = torch.cat((c_encoded, kld_encoded), dim=1)
        #self.out = self.mlp_model(z_concat)
        self.out = self.mlp_model(kld_encoded)
    ...
    def eval_plot(self, x_tensor_cuda, y_bin, epoch, args):
        with torch.no_grad():
            ...
            for i in range(0, x_encoded.shape[1], 2):
                ...
                directory_path = "./maps/triplet_only_kld/"+ str(x_encoded.shape[1]) + "/epoch_" + str(epoch)
