...
class TripletSampler(Sampler):
    ...
    def __init__(self, labels, batch_size):
        ...
        self.all_indices = np.arange(len(labels))
        # sample half of the batch_size as self.length_of_single_pass
        self.length_of_single_pass = self.batch_size // 3 # 512
        self.batch_num = len(labels) // self.length_of_single_pass + 1 # 30533 // 512 + 1 = 60
        self.list_size = len(labels) * 3 # 30533 * 3
        assert self.list_size >= self.batch_size
    ...
    def __iter__(self):
        idx_list = [0] * self.list_size
        num_iters = self.calculate_num_iters() # batch_num, 60
